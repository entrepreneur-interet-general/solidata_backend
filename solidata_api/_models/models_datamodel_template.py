...
"""
_models/models_datamodel_template.py  
"""

from log_config import log, pformat

log.debug("... loading models_datamodel_template.py ...")
...
class NewDmt : 
	"""
	Model to display / marshal dmt basic form
	"""

	def __init__(self, ns_):
		self.mod = ns_.model( "Dmt_basics", {**doc_basics, **open_level_edit_} )
...
class Dmt_infos : 
	...
	def __init__(self, ns_) :
		...
		self.uses_light				= create_model_uses(		ns_,	model_name=model_type+"_uses", 		schema_list=[ "dmt" ])
		
		self.datasets 				= create_model_datasets(	ns_, 	model_name=model_type+"_datasets", 	schema_list=[ "dmf","tag" ])
		self.datasets_light			= create_model_datasets(	ns_, 	model_name=model_type+"_datasets", 	schema_list=[ "dmf","tag" ], is_light=True )
